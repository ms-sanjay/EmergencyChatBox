...
// LoRa pins (adjust to your wiring)
#define LORA_SS  15    // GPIO5 (D1)
#define LORA_RST 16   // GPIO14 (D5)
#define LORA_DIO0 4   // GPIO2 (D4)
...
void onWebSocketEvent(uint8_t num, WStype_t type, uint8_t * payload, size_t length) {
  ...
  for (uint8_t i = 0; i < 4; i++) {
    if (i != num && webSocket.clientIsConnected(i)) {
      webSocket.sendTXT(i, msg);
    }
  }
  // Send the same message over LoRa to other nearby nodes
  sendLoRaMessage(msg);
  break;
}
    default:
...
void setup() {
  ...
  httpServer.on("/generate_204", captivePortal);        // Android captive portal detection
  httpServer.on("/hotspot-detect.html", captivePortal); // iOS captive portal detection
  httpServer.on("/fwlink", captivePortal);              // Windows captive portal detection
  httpServer.on("/ncsi.txt", captivePortal);            // Windows captive portal detection
...
void loop() {
  ...
  if (packetSize) {
    ...
    Serial.println("LoRa received: " + incomingMsg);

    // Broadcast received LoRa message to local WebSocket clients
    webSocket.broadcastTXT(incomingMsg);
  }
}
